            ...
            case 'MODE':
                ...
                modeList.forEach(function(mode) {
                    ...
                    if (mode in self.prefixForMode) {
                        ...
                        if (adding) {
                            if (channel.users[modeArg] != null && channel.users[modeArg].indexOf(self.prefixForMode[mode]) === -1) {
                                channel.users[modeArg] += self.prefixForMode[mode];
                            }
                        }
                        else {
                            if (channel.users[modeArg]) {
                                channel.users[modeArg] = channel.users[modeArg].replace(self.prefixForMode[mode], '');
                            }
                        }
                    }
                    else {
                        var hasModeArg = false;
                        var listMode = self.supported.channel.modes["a"].includes(mode);
                        if (adding && (
                            listMode ||
                            self.supported.channel.modes["b"].includes(mode) ||
                            self.supported.channel.modes["c"].includes(mode)
                        )) {
                            hasModeArg = true;
                        }
                        else if (!adding && (
